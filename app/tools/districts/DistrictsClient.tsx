        ...
        <div className="mt-4 pt-3 flex items-center justify-between" style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          <div className="ml-4 flex gap-3 shrink-0">
            {matchup && (() => {
              ...
              return q ? (
                <Link href={`/races?q=${encodeURIComponent(q)}`}
...
export default function DistrictsPage() {
  ...
  // Shared with PathToWin's own fetch; the browser HTTP cache dedupes it.
  const [races, setRaces] = useState<DistrictRacesFile | null>(null);
  useEffect(() => {
    fetch("/data/district-races.json").then(r => r.ok ? r.json() : null).then(d => { if (d) setRaces(d); }).catch(() => {});
  }, []);
    ...
    <div className="districts-page" style={{ background: "#F1F2EE", minHeight: "100vh", fontFamily: "var(--font-outfit,sans-serif)" }}>
      ...
      <section className="relative overflow-hidden topo-hero"
        ...
        <div className="relative max-w-7xl mx-auto px-5">
          ...
          {(() => {
            /* Live share card: same bar/duel/badge the server metadata builds,
               so the dialog preview matches what a crawler renders. */
            const dKey = district ? `${type.toUpperCase()}-${district}` : null;
            const m = dKey ? getMatchup(dKey) : null;
            const badge = m?.lean
              ? m.lean.replace(/-d$/, " D").replace(/-r$/, " R").replace(/^(\w)/, c => c.toUpperCase()).replace(/-/g, " ")
              : undefined;
            const dSide = m?.sides.find(x => x.party === "D");
            const rSide = m?.sides.find(x => x.party === "R");
            const dCash = dSide ? getFinanceByName(dSide.name)?.cash ?? 0 : 0;
            const rCash = rSide ? getFinanceByName(rSide.name)?.cash ?? 0 : 0;
            const duel = dSide && rSide && dCash + rCash > 0
              ? { dName: dSide.name, dCash, rName: rSide.name, rCash }
              : undefined;
            const lastGen = races && district && (type === "hd" || type === "sd" || type === "cd")
              ? findLastContestedGeneral(races, type, district) : null;
            const bar = lastGen ? {
              dLabel: `D ${lastGen.year}`,
              dPct: lastGen.winnerParty === "D" ? lastGen.winnerPct : lastGen.loserPct,
              rLabel: `R ${lastGen.year}`,
              rPct: lastGen.winnerParty === "R" ? lastGen.winnerPct : lastGen.loserPct,
            } : undefined;
            return (
          <ShareButton
            toolName={district ? `${headerLabel(type, district)} · District portrait` : "Districts"}
            section="Elections"
